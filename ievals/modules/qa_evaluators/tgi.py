        ...
        for row_index, row in tqdm(test_df.iterrows(), total=len(test_df), dynamic_ncols=True):
            ...
            for prompt in full_prompt:
                if prompt["role"] == "system":
                    text += self.systemMessageToken + prompt["content"] + self.messageEndToken
                elif prompt["role"] == "user":
                    text += self.userMessageToken + prompt["content"] + self.messageEndToken
                elif prompt["role"] == "assistant":
                    text += self.assistantMessageToken + prompt["content"] + self.messageEndToken
            ...
            if response == None:
                ...
                response_str = response.json()["generated_text"].split(self.messageEndToken)[0]
            if cot:
